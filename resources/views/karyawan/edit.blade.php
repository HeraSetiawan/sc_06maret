...
@section('konten')
    <div class="col-8 mx-auto mt-3">
        <div class="card shadow">
            <div class="card-body">
                <div class="card-title fs-3 fw-bold">Form Update Karyawan</div>
                <form action="{{ url('/karyawan',$karyawan->id) }}" method="post">
                    @method('put')
                    @csrf
                    <div class="mb-3">
                        <label class="form-label">Nama</label>
                        <input value="{{ $karyawan->nama }}" type="text" class="form-control @error('nama') is-invalid @enderror" name="nama" placeholder="masukan nama karyawan">
                        @error('nama')
                            ...
                            <div class="mb-3">
                                ...
                                <select name="jabatan_id" id="" class="form-select">
                                    ...
                                    @foreach ($jabatan as $jb)
                                        <option @selected($karyawan->jabatan_id == $jb->id) value="{{ $jb->id }}">{{ $jb->nama }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
